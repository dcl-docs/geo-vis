...
Here's an example basemap from Mapbox, who basemaps and tools we'll use in this chapter.
...
And here's what it look like if we zoom on Dakar.
...
In this chapter, we'll use [Mapbox](https://www.mapbox.com/) to create both interactive and static maps that include basemaps. We will beging by using Mapbox Studio to create interactive maps and then use the Mapbox Static API to turn these interactive maps into PNG images.
...
Vector tiles store all data in vector format. This means that they can distinguish between different _layers_, such as roads, country borders, and lakes, and you can easily manipulate these elements before rendering. Vector tiles also make operations like tilting possible. If you wanted to change the color of the highways in a vector tileset, all you'd have to do is change the way that highways layer is displayed, which is fast and easy with vector data. Below, we'll walk you through the creation of two interactive Mapbox maps. When you're done creating each map, you'll have created a custom vector tileset with various layers and a set of rules for styling these layers.
...
* The two tilesets should now appear in the list. Each tileset will have a random string identifier after its name. If your tilesets did not appear, try refreshing your browser.
...
* When you publish your map, you'll get a URL that points to your published map. Your published map will have a starting location and zoom level. By default, this location and zoom level will the location and zoom level of your editing map when you click __Publish__. Before publishing, make sure you're happy with the current location and zoom level.
* You can see the exact longitude, latitude, and zoom level by clicking on _Map position_.
...
* The _Share URL_ points to the shareable version of your map. Copy this URL and paste it into a new browser tab. Your map should open at the default location and zoom level that you set earlier. The Share URLs take the following form:
...
* Once you have the correct Share URL, copy it and store it somewhere. You'll also need this URL for the static API later on.
...
* Right now, the `deaths` variable is not mapped to the intensity of the heatmap. If you zoom in on your heatmap, you'll notice that each heatmap dot has the same intensity, regardless of the number of deaths at  that location. To map `deaths` to intensity, we'll use the `Weight` parameter. We'll adjust the heatmap weights similarly to how we adjusted the radii of the circles for the previous map. 
* Click on _Weight_, _Style across data range_, then _deaths_ and _Done_. This maps `deaths` to the weight, but Studio default to keeping the relationship uniform, so the weight doesn't actually change across different values of `deaths`. 
...
* Now, adjust the heatmap radius in the __Radius__ tab. _Radius_ controls the radii of the individual heatmap circles. Experiment with different values.
...
* Just like you did when finalizing your circle map, adjust the longitude, latitude, and zoom of your map so that your published map defaults to the correct location and zoom level.
* Click the blue __Publish__ button on the right to publish your map. Then, choose __Publish__.
* Change your map to _Public_.
* Copy the Share URL. Again, paste this URL into a new browser tab to check that your map defaults to the correct location. If your map does not default to the location you set earlier, you may have to manually change the Share URL by following the steps we explained in 1.3.9.
* Copy the Share URL and store it somewhere. You'll need it for the next section.

## Static maps

Right now, you can share your interactive Mapbox maps via the URL. Sometimes, you'll want a static version of your map to embed in documents that only support static images. In this section, you'll learn how to create static versions of your two maps using the Mapbox Static API.
...
You create the API call by specifying parameters in a URL. Here's what the basic API call looks like:

`https://api.mapbox.com/styles/v1/{username}/{style_id}/static/{longitude},{latitude},{zoom}/{width}x{height}?access_token={access_token}`

This call will create a PNG version of your map.
...
Decide whether you want to first create a static version of your circles map or your heatmap. 

Now, we need to find this map's style ID. You can find the style ID in the Share URL you found earlier. 
...
Note that leaflet.mapboxgl requires RStudio version 1.2 or later to display your plots. 
...
* Your style URL is different than the share URL you found earlier. To find your style URL, first go to the [styles page](https://studio.mapbox.com/). Then, click on __Menu__ next to your style and copy the style URL. 
